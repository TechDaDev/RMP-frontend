export const API_ENDPOINTS = {
  health: {
    root: "/api/health/",
    live: "/api/health/live/",
    ready: "/api/health/ready/",
    deps: "/api/health/deps/",
  },
  docs: {
    schema: "/api/schema/",
    swagger: "/api/docs/",
  },
  accounts: {
    register: "/api/accounts/register/",
    activate: "/api/accounts/activate/",
    resendActivationOtp: "/api/accounts/resend-activation-otp/",
    login: "/api/accounts/login/",
    me: "/api/accounts/me/",
    passwordResetRequest: "/api/accounts/password-reset/request/",
    passwordResetConfirm: "/api/accounts/password-reset/confirm/",
    tokenRefresh: "/api/accounts/token/refresh/",
    logout: "/api/accounts/logout/",
  },
  profiles: {
    me: "/api/profiles/me/",
    userProfile: "/api/profiles/me/user-profile/",
    patient: "/api/profiles/me/patient/",
    doctor: "/api/profiles/me/doctor/",
    pharmacist: "/api/profiles/me/pharmacist/",
    laboratorian: "/api/profiles/me/laboratorian/",
  },
  consultations: {
    create: "/api/consultations/",
    my: "/api/consultations/my/",
    doctorPending: "/api/consultations/doctor/pending/",
    doctorAssigned: "/api/consultations/doctor/assigned/",
    detail: (id: string) => `/api/consultations/${id}/`,
    accept: (id: string) => `/api/consultations/${id}/accept/`,
    close: (id: string) => `/api/consultations/${id}/close/`,
    responses: (id: string) => `/api/consultations/${id}/responses/`,
    messages: (id: string) => `/api/consultations/${id}/messages/`,
    markMessagesRead: (id: string) => `/api/consultations/${id}/messages/mark-read/`,
    symptomCategories: "/api/consultations/symptom-categories/",
    symptoms: "/api/consultations/symptoms/",
  },
  doctorConsultations: {
    pending: "/api/consultations/doctor/pending/",
    assigned: "/api/consultations/doctor/assigned/",
    detail: (id: string) => `/api/consultations/${id}/`,
    accept: (id: string) => `/api/consultations/${id}/accept/`,
    response: (id: string) => `/api/consultations/${id}/responses/`,
    close: (id: string) => `/api/consultations/${id}/close/`,
    messages: (id: string) => `/api/consultations/${id}/messages/`,
    markMessagesRead: (id: string) => `/api/consultations/${id}/messages/mark-read/`,
  },
  prescriptions: {
    my: "/api/prescriptions/my/",
    myDetail: (id: string) => `/api/prescriptions/my/${id}/`,
    byDoctor: (id: string) => `/api/prescriptions/doctor/${id}/`,
    cancelByDoctor: (id: string) => `/api/prescriptions/doctor/${id}/cancel/`,
    createForConsultation: (consultationId: string) => `/api/consultations/${consultationId}/prescriptions/`,
    scan: "/api/prescriptions/scan/",
    dispense: (id: string) => `/api/prescriptions/${id}/dispense/`,
  },
  doctorPrescriptions: {
    createFromConsultation: (consultationId: string) => `/api/consultations/${consultationId}/prescriptions/`,
    detail: (prescriptionId: string) => `/api/prescriptions/doctor/${prescriptionId}/`,
    cancel: (prescriptionId: string) => `/api/prescriptions/doctor/${prescriptionId}/cancel/`,
  },
  pharmacistPrescriptions: {
    scan: "/api/prescriptions/scan/",
    detail: (prescriptionId: string) => `/api/prescriptions/${prescriptionId}/`,
    dispense: (prescriptionId: string) => `/api/prescriptions/${prescriptionId}/dispense/`,
    history: "/api/prescriptions/pharmacist/history/",
  },
  labOrders: {
    tests: "/api/lab-orders/tests/",
    my: "/api/lab-orders/my/",
    myDetail: (id: string) => `/api/lab-orders/my/${id}/`,
    byDoctor: (id: string) => `/api/lab-orders/doctor/${id}/`,
    cancelByDoctor: (id: string) => `/api/lab-orders/doctor/${id}/cancel/`,
    createForConsultation: (consultationId: string) => `/api/consultations/${consultationId}/lab-orders/`,
    scan: "/api/lab-orders/scan/",
    complete: (id: string) => `/api/lab-orders/${id}/complete/`,
    resultDetail: (id: string) => `/api/lab-orders/results/${id}/`,
    doctorResultDetail: (id: string) => `/api/lab-orders/doctor/results/${id}/`,
    doctorResultReview: (id: string) => `/api/lab-orders/doctor/results/${id}/review/`,
    doctorResultRelease: (id: string) => `/api/lab-orders/doctor/results/${id}/release/`,
    doctorResultLinkMedicalRecord: (id: string) => `/api/lab-orders/doctor/results/${id}/link-medical-record/`,
    createItemResult: (itemId: string) => `/api/lab-orders/items/${itemId}/results/`,
    correctResult: (id: string) => `/api/lab-orders/results/${id}/correct/`,
  },
  laboratoryTests: {
    list: "/api/lab-orders/tests/",
  },
  laboratoryOrders: {
    scan: "/api/lab-orders/scan/",
    complete: (orderId: string) => `/api/lab-orders/${orderId}/complete/`,
  },
  laboratoryResults: {
    createForItem: (itemId: string) => `/api/lab-orders/items/${itemId}/results/`,
    detail: (resultId: string) => `/api/lab-orders/results/${resultId}/`,
    correct: (resultId: string) => `/api/lab-orders/results/${resultId}/correct/`,
  },
  doctorLabOrders: {
    createFromConsultation: (consultationId: string) => `/api/consultations/${consultationId}/lab-orders/`,
    detail: (labOrderId: string) => `/api/lab-orders/doctor/${labOrderId}/`,
    cancel: (labOrderId: string) => `/api/lab-orders/doctor/${labOrderId}/cancel/`,
  },
  doctorLabResults: {
    detail: (labResultId: string) => `/api/lab-orders/doctor/results/${labResultId}/`,
    review: (labResultId: string) => `/api/lab-orders/doctor/results/${labResultId}/review/`,
    release: (labResultId: string) => `/api/lab-orders/doctor/results/${labResultId}/release/`,
    linkMedicalRecord: (labResultId: string) => `/api/lab-orders/doctor/results/${labResultId}/link-medical-record/`,
  },
  labResults: {
    my: "/api/lab-results/my/",
    myDetail: (id: string) => `/api/lab-results/my/${id}/`,
    myFromLabOrders: "/api/lab-orders/my-results/",
    myDetailFromLabOrders: (id: string) => `/api/lab-orders/my-results/${id}/`,
  },
  catalog: {
    drugs: "/api/catalog/drugs/",
    drugDetail: (id: string) => `/api/catalog/drugs/${id}/`,
    labTests: "/api/catalog/lab-tests/",
    labTestDetail: (id: string) => `/api/catalog/lab-tests/${id}/`,
  },
  pharmacyInventory: {
    list: "/api/pharmacy/inventory/",
    detail: (id: string) => `/api/pharmacy/inventory/${id}/`,
  },
  pharmacyRequests: {
    list: "/api/pharmacy/requests/",
    detail: (id: string) => `/api/pharmacy/requests/${id}/`,
    quote: (id: string) => `/api/pharmacy/requests/${id}/quote/`,
    accept: (id: string) => `/api/pharmacy/requests/${id}/accept/`,
    reject: (id: string) => `/api/pharmacy/requests/${id}/reject/`,
    cancel: (id: string) => `/api/pharmacy/requests/${id}/cancel/`,
    complete: (id: string) => `/api/pharmacy/requests/${id}/complete/`,
  },
  labInventory: {
    list: "/api/lab/inventory/",
    detail: (id: string) => `/api/lab/inventory/${id}/`,
  },
  labRequests: {
    list: "/api/lab/requests/",
    detail: (id: string) => `/api/lab/requests/${id}/`,
    quote: (id: string) => `/api/lab/requests/${id}/quote/`,
    accept: (id: string) => `/api/lab/requests/${id}/accept/`,
    reject: (id: string) => `/api/lab/requests/${id}/reject/`,
    cancel: (id: string) => `/api/lab/requests/${id}/cancel/`,
    complete: (id: string) => `/api/lab/requests/${id}/complete/`,
  },
  payments: {
    wallet: "/api/payments/wallet/",
    walletTransactions: "/api/payments/wallet/transactions/",
    adminWallets: "/api/payments/admin/wallets/",
    adminManualRecharge: "/api/payments/admin/manual-recharge/",
    intents: "/api/payments/intents/",
    intentDetail: (id: string) => `/api/payments/intents/${id}/`,
    payWallet: (id: string) => `/api/payments/intents/${id}/pay-wallet/`,
  },
  patientRecords: {
    my: "/api/patient-records/my/",
    byPatient: (patientId: string) => `/api/patient-records/patients/${patientId}/`,
    createEntry: (recordId: string) => `/api/patient-records/${recordId}/entries/`,
    confirmEntry: (entryId: string) => `/api/patient-records/entries/${entryId}/confirm/`,
    deactivateEntry: (entryId: string) => `/api/patient-records/entries/${entryId}/deactivate/`,
    setBloodGroup: (recordId: string) => `/api/patient-records/${recordId}/blood-group/`,
    verifyBloodGroup: (patientId: string) => `/api/patient-records/patients/${patientId}/blood-group/verify/`,
  },
  doctorPatientRecords: {
    detail: (patientId: string) => `/api/patient-records/patients/${patientId}/`,
  },
  notifications: {
    list: "/api/notifications/",
    unreadCount: "/api/notifications/unread-count/",
    markRead: (id: string) => `/api/notifications/${id}/mark-read/`,
    markAllRead: "/api/notifications/mark-all-read/",
  },
  knowledgeBase: {
    documents: "/api/knowledge-base/documents/",
    documentDetail: (id: string) => `/api/knowledge-base/documents/${id}/`,
    process: (id: string) => `/api/knowledge-base/documents/${id}/process/`,
    approve: (id: string) => `/api/knowledge-base/documents/${id}/approve/`,
    reject: (id: string) => `/api/knowledge-base/documents/${id}/reject/`,
    archive: (id: string) => `/api/knowledge-base/documents/${id}/archive/`,
    embed: (id: string) => `/api/knowledge-base/documents/${id}/embed/`,
    chunksByDocument: (id: string) => `/api/knowledge-base/documents/${id}/chunks/`,
    chunksSearch: "/api/knowledge-base/chunks/search/",
    chunksSemanticSearch: "/api/knowledge-base/chunks/semantic-search/",
  },
  rag: {
    doctorQuery: "/api/rag/doctor/query/",
    consultationSupport: (id: string) => `/api/rag/consultations/${id}/support/`,
    labResultSupport: (id: string) => `/api/rag/lab-results/${id}/support/`,
    feedbackCreate: (id: string) => `/api/rag/responses/${id}/feedback/`,
    myFeedback: "/api/rag/feedback/my/",
    adminFeedback: "/api/rag/admin/feedback/",
    adminReviewFeedback: (id: string) => `/api/rag/admin/feedback/${id}/review/`,
    adminAnalyticsSummary: "/api/rag/admin/analytics/summary/",
    adminExportDataset: "/api/rag/admin/exports/dataset/",
  },
  ragDoctorAssistant: {
    consultationMessages: (consultationId: string) =>
      `/api/rag/consultations/${consultationId}/doctor-ai-messages/`,
    generateFromMedicalReport: (reportId: string) =>
      `/api/rag/medical-reports/${reportId}/doctor-ai-message/`,
    detail: (messageId: string) => `/api/rag/doctor-ai-messages/${messageId}/`,
    markRead: (messageId: string) => `/api/rag/doctor-ai-messages/${messageId}/mark-read/`,
  },
  admin: {
    knowledgeDocuments: "/api/knowledge-base/documents/",
    knowledgeDocumentDetail: (id: string) => `/api/knowledge-base/documents/${id}/`,
    knowledgeDocumentProcess: (id: string) => `/api/knowledge-base/documents/${id}/process/`,
    knowledgeDocumentApprove: (id: string) => `/api/knowledge-base/documents/${id}/approve/`,
    knowledgeDocumentReject: (id: string) => `/api/knowledge-base/documents/${id}/reject/`,
    knowledgeDocumentArchive: (id: string) => `/api/knowledge-base/documents/${id}/archive/`,
    knowledgeDocumentEmbed: (id: string) => `/api/knowledge-base/documents/${id}/embed/`,
    knowledgeDocumentChunks: (id: string) => `/api/knowledge-base/documents/${id}/chunks/`,
    ragFeedback: "/api/rag/admin/feedback/",
    ragFeedbackReview: (id: string) => `/api/rag/admin/feedback/${id}/review/`,
    ragAnalyticsSummary: "/api/rag/admin/analytics/summary/",
    ragDatasetExport: "/api/rag/admin/exports/dataset/",
    verifications: "/api/admin/verifications/",
    verificationDetail: (role: string, id: string) => `/api/admin/verifications/${role}/${id}/`,
    verificationApprove: (role: string, id: string) => `/api/admin/verifications/${role}/${id}/approve/`,
    verificationReject: (role: string, id: string) => `/api/admin/verifications/${role}/${id}/reject/`,
    verificationSuspend: (role: string, id: string) => `/api/admin/verifications/${role}/${id}/suspend/`,
  },
  websocket: {
    user: "/ws/user/",
    consultationMessages: (consultationId: string) => `/ws/consultations/${consultationId}/messages/`,
  },
} as const;

export type ApiEndpoints = typeof API_ENDPOINTS;
