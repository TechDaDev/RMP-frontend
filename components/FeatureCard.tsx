...
export function FeatureCard({ title, desc, icon }: FeatureCardProps) {
  return (
    <article className="group rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-[var(--card-shadow)] transition duration-300 hover:-translate-y-1.5 hover:border-[var(--color-primary)] hover:shadow-[var(--card-shadow-lg)]">
      {icon != null && (
        <span className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-[color:color-mix(in_srgb,var(--color-primary)_10%,var(--color-surface-alt))] text-[var(--color-primary)] transition group-hover:bg-[color:color-mix(in_srgb,var(--color-primary)_18%,var(--color-surface-alt))]">
          {icon}
        </span>
      )}
