...
export default function PatientLabRequestsPage() {
  ...
  return (
    <PatientPageFrame>
      <PageHeader title={t.patient.labServiceRequestsTitle} description={t.patient.labServiceRequestsSubtitle} />
      <Card className="space-y-2">
        {items.length === 0 ? <p className="text-sm text-[var(--color-muted)]">{t.patient.noLabRequests}</p> : null}
        {items.map((item) => (
          <div key={item.id} className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-[var(--color-border)] p-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href={`/app/patient/lab-requests/${item.id}`} className={buttonClassName({ variant: "secondary" })}>{t.patient.openRequest}</Link>
            </div>
          </div>
        ))}
      </Card>
