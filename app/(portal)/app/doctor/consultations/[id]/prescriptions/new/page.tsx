...
import type { CreateDoctorPrescriptionRequest, DoctorConsultationDetail } from "@/types/doctor";
...
export default function DoctorCreatePrescriptionPage() {
  ...
  const canCreate = Boolean(consultation && canDoctorCreatePrescription(consultation.status) && isApproved);
  ...
  async function handleCreatePrescription(payload: CreateDoctorPrescriptionRequest) {
    const created = await createPrescriptionFromConsultation(consultationId, payload);
    if (created.id) {
      router.replace(`/app/doctor/prescriptions/${created.id}`);
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        badge={<Badge tone="primary">{t.doctor.createPrescription}</Badge>}
      ...
      {error || !consultation ? (
        ...
        <DoctorPrescriptionForm onSubmit={handleCreatePrescription} />
      ) : (
        <Card className="space-y-4 rounded-[2rem]">
          <EmptyState
            title={t.doctor.createPrescription}
            description={!isApproved ? t.doctor.verifiedDoctorRequiredDescription : t.doctor.prescriptionRequiresAcceptedConsultation}
          />
        </Card>
      )}
